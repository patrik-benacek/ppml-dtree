...
for (stat_id in st_meta$wmo_id){
  filestats = Sys.glob(file.path(data_dir, 'observations/data/', years_regex, paste0("prec24_obs_stat_", stat_id, "_*.rds")))
...
for(filename in fc_files){
  ...
  for(thisst in stationlist){
    ...
    for(i in 1:seq_along(fc_raw_validtime_append)){
      vtime = fc_raw_validtime_append[i] 
      ...
      # There are duplicities in observations
        ...
        obs_temp_append[thisst_pos,vtime_pos] <- st_temp$prec24[st_temp_pos]
      }else{
        obs_temp_append[thisst_pos,vtime_pos] <- st_temp$prec24[st_temp_pos]
      }
      
      # choose forecasts from nearest grid point
      fc_interpolated_temp_append[thisst_pos, , vtime_pos] <- fc_raw_temp[lon_pos_mindiff, lat_pos_mindiff, , fc_time_pos]
    }
  }
...
dlname <- "interpolated 24h precip ensemble forecast"
prec24_fc_def <- ncvar_def(name = "prec24_fc", units = "mm/24h",
                        dim = list(stationdim,memberdim,timedim),
                        missval = fillvalue, longname = dlname,
                        prec="single")

dlname <- "24h precip station observation"
prec24_obs_def <- ncvar_def(name = "prec24_obs", units = "mm/24h",
...
# put variables
ncvar_put(nc = ncout, varid = prec24_fc_def, vals = fc_interpolated_temp)
ncvar_put(nc = ncout, varid = prec24_obs_def, vals = obs_temp)
